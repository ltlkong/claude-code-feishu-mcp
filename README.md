...
飞书通过 WebSocket 推送消息到 MCP Server，Server 将消息注入 Claude Code 的对话流，Claude 通过 MCP 工具（`reply`、`reply_card`、`reply_file` 等）实时回复飞书用户。
...
| `HEARTBEAT_INTERVAL_MINUTES` | 否 | `15` | Heartbeat 轮询间隔（分钟） |
| `HEARTBEAT_INACTIVITY_MINUTES` | 否 | `5` | 用户不活跃多久后触发 Heartbeat |
| `TENOR_API_KEY` | 否 | | Tenor GIF 搜索 API key |
...
| 工具 | 说明 |
|------|------|
| `reply(chat_id, text)` | 发送纯文本消息（可多次调用，像真人打字） |
| `reply_card(request_id, status, text, done, emoji, template)` | 发送/更新飞书卡片（done=false 更新进度，done=true 关闭卡片） |
