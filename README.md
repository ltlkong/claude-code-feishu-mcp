...
- **文件收发** — 图片/文档/音频双向传输，Claude 直接分析
- **定时提醒** — 通过 cron 创建定时消息和智能任务
- **飞书卡片 V2** — 结构化回复用交互式卡片，支持按钮操作
- **Heartbeat** — 定期回顾对话历史，有值得说的事主动发消息
...
| ffmpeg | 任意 | `brew install ffmpeg` | `ffmpeg -version` | **可选** — 语音消息需要 |

### 2. 安装项目

```bash
git clone https://github.com/joewongjc/claude-code-feishu-mcp.git
...
左侧菜单「事件与回调」→「事件配置」→ 订阅方式选择「**使用长连接接收事件**」（不是 Webhook）→ 添加事件 `im.message.receive_v1`
...
项目已通过 `.mcp.json` 注册为 Claude Code 的 MCP Server，在项目目录下直接运行：

```bash
claude
```

Claude Code 启动时会自动加载 feishu MCP Server，飞书消息即刻可达。
...
在飞书中给机器人发消息，Claude 会实时以卡片形式回复。支持私聊和群聊（群聊需 @机器人）。
...
| `HEARTBEAT_INTERVAL_MINUTES` | 否 | `60` | Heartbeat 检查间隔（分钟） |
...
├── deploy/
│   ├── feishu-claude.plist   # macOS launchctl 配置
│   └── feishu-claude.service # Linux systemd 配置
...
# 编辑 plist，把 /path/to/claude-code-feishu-mcp 改为实际路径
...
# 编辑 service，修改 WorkingDirectory、User、EnvironmentFile 路径
...
- Streaming card output (real-time typing effect via Feishu Card V2)
...
- Cron-based reminders with smart task support
- Heartbeat — proactive messaging based on conversation history
- Works alongside other MCP servers (browser, gmail, etc.)
...
| ffmpeg | `brew install ffmpeg` | No (voice only) |
