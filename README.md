...
- **智能 Heartbeat** — 用户不活跃 30 分钟后才触发，活跃时不打扰
- **消息 Debounce** — 2 秒内连发多条合并为一个卡片
...
| `HEARTBEAT_INTERVAL_MINUTES` | 否 | `60` | Heartbeat 轮询间隔（分钟） |
| `HEARTBEAT_INACTIVITY_MINUTES` | 否 | `30` | 用户不活跃多久后触发 Heartbeat |
...
- Message debounce — rapid messages merged into single card
